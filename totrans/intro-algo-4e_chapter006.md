...
+   运行时间为*O*(*n* lg *n*)的HEAPSORT过程可以原地对数组进行排序。
...
[图 6.4](chapter006.xhtml#Fig_6-4) 展示了在第 1 行构建初始最大堆后 HEAPSORT 操作的示例。该图显示了在行 2–5 的 **for** 循环的第一次迭代之前和每次迭代之后的最大堆。
...
你可以假设在调用MAX-HEAP-INCREASE-KEY时，子数组*A*[1 : *A.heap-size*]满足最大堆属性。
