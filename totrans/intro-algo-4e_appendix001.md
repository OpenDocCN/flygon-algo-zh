...
这个级数的(*k*+1)项和*k*项的比值是*k*/(*k*+1) < 1，但这个级数不受递减几何级数的限制。要通过几何级数限制一个级数，你需要证明存在一个小于1的*r*，即一个*常数*，使得所有连续项的比值永远不超过*r*。在调和级数中，不存在这样的*r*，因为比值会无限接近1。
