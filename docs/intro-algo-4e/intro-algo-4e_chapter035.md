...
贪心算法的工作方式如下。在每次迭代开始时，*U[i]*是包含剩余未覆盖元素的*X*的子集，初始子集*U*[0]包含*X*中的所有元素。集合 C 包含正在构建的子族。第 5 行是贪心决策步骤，选择尽可能覆盖尽可能多未覆盖元素的子集*S*（任意打破平局）。选择*S*后，第 6 行更新剩余未覆盖元素的集合，用*U*[*i*+1]表示，第 7 行将*S*放入 C。当算法终止时，C 是覆盖*X*的*ℱ*的子族。
