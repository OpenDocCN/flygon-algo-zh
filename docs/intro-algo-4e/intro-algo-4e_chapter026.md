...
让`T1`表示 P-NAIVE-MERGE-SORT 在一个`n`元素子数组上的（最坏情况）工作量，其中`n` = `r` −`p` + 1 是`A[p : r]`中元素的数量，并且让`T∞`表示跨度。因为 MERGE 是串行的，运行时间为Θ(`n`)，它的工作量和跨度都是Θ(`n`)。由于 P-NAIVE-MERGE-SORT 的串行投影恰好是 MERGE-SORT，其工作量为`T1` = Θ(`n` lg `n`)。第 5 行和第 7 行中的两个递归调用并行运行，因此其跨度由递归给出
...
***a.*** 设计并分析一个并行算法`P-REDUCE`，使用递归生成来执行相同的功能，具有Θ(`n`)的工作量和Θ(lg *n*)的跨度。
