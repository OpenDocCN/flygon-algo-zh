...
考虑对存储在数组`A[1：n]`中的`n`个数字进行排序，首先找到`A[1：n]`中最小的元素并将其与`A₁`中的元素交换。然后找到`A[2：n]`中最小的元素，并将其与`A₂`交换。然后找到`A[3：n]`中最小的元素，并将其与`A₃`交换。以此类推，对`A`的前`n - 1 `个元素继续这样做。为这种算法编写伪代码，这种算法被称为**选择排序**。这个算法维护什么循环不变式？为什么它只需要运行前`n - 1 `个元素，而不是所有`n`个元素？给出选择排序的最坏情况运行时间的Θ-符号。最佳情况的运行时间是否更好？
