...
（稍后我们将看到这种假设提供的优势。）例如，考虑图 15.1 中的活动集合。子集{`a₃`，`a[9]`，`a[11]`}由相互兼容的活动组成。然而，它不是最大的子集，因为子集{`a₁`，`a[4]`，`a[8]`，`a[11]`}更大。事实上，{`a₁`，`a[4]`，`a[8]`，`a[11]`}是最大的相互兼容活动子集，另一个最大的子集是{`a₂`，`a[4]`，`a[9]`，`a[11]`}。
