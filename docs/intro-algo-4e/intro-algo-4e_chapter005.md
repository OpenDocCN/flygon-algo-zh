...
举例来说，在 `n = 1`000 次抛硬币中，至少出现 2 ⌈lg n⌉ = 20 个连续正面朝上的概率最多为 1/n = 1/1000。至少出现 3 ⌈lg n⌉ = 30 个连续正面朝上的概率最多为 1/n² = 1/1,000,000。  
...
**b.** 计数器表示的计数方差的分析取决于`n[i]`的序列。让我们考虑一个简单的情况：对于所有`i ≥ 0`，`n[i] = 1`00`i`。在执行`n`次增量操作后，估计寄存器表示的值的方差。
