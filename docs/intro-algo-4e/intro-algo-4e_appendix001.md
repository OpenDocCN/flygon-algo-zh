...
给定一个数字序列*a*[1]，*a*[2]，…，*a[n]*，其中*n*是非负整数，有限和*a*[1] + *a*[2] + … + *a[n]*可以表示为![art](img/Art_P1526.jpg)。如果*n* = 0，求和的值被定义为 0。有限级数的值总是明确定义的，其项的加法顺序并不重要。
...
有时可以通过改变其索引来简化一个级数，通常是颠倒求和的顺序。考虑级数 ![art](img/Art_P1548.jpg)。因为这个求和中的项是 *a[n]*, *a*[*n*−1], … , *a*[0]，我们可以通过令 *j* = *n* − *k* 来颠倒索引的顺序，并将这个求和重写为
...
通过使用求和的线性性质证明 ![art](img/Art_P1554.jpg)。

***A.1-2***

找到一个简单的公式来表示 ![art](img/Art_P1555.jpg)。
...
你并不总是需要猜测求和的确切值才能使用数学归纳法。相反，你可以使用归纳法证明求和的上界或下界。举个例子，让我们证明渐近上界 ![art](img/Art_P1567.jpg)。更具体地，我们将证明对于某个常数 *c*，有 ![art](img/Art_P1568.jpg)。对于初始条件 *n* = 0，只要 *c* ≥ 1，我们有 ![art](img/Art_P1569.jpg)。假设该界对于 *n* 成立，我们证明它对于 *n* + 1 也成立。我们有
...
你可以应用这种方法来限制求和![艺术](img/Art_P1581.jpg)。为了从*k* = 0 开始求和，将其重写为![艺术](img/Art_P1582.jpg)。第一项(*a*[0])是 1/3，连续项的比值(*r*)是
...
对于由算法分析产生的求和，有时你可以将求和分割并忽略初始项的常数数量。通常，当求和![艺术](img/Art_P1589.jpg)中的每一项*a[k]*与*n*无关时，这种技术适用。然后对于任意常数*k*[0] > 0，你可以写成
