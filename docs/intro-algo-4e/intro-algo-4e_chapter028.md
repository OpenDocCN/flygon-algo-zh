...
| `l[31]y₁ + l[32]y₂ + y₃` | = | `b[π₃]，` |
|  | ⋮ |  |   |
| `l[n1]*y₁ + l[n2]*y₂ + l[n3]*y₃ + ⋯ + y[n]` | = | `b[π[n]]`。 |   |
...
首先计算`y₁`，然后计算`y₂`，最后计算`y₃`。然后，使用回代法解出`Ux = y`的`x`：
...
`c₁ + c₂*x + c₃*x² + c[4] sin(2*πx) + c[5] cos(2*πx)`
