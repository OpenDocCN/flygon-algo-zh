...
如果存在系数 `c₁`, `c₂`, … , `c[n]`，不全为 0，使得 `c₁*x₁ + c₂*x₂ + ⋯ + c[n]x[n] = 0`，那么向量 `x₁`, `x₂`, … , `x[n]` **线性相关**。例如，行向量 `x₁ = ( 1 2 3 )`，`x₂ = ( 2 6 4 )`，和 `x₃ = ( 4 11 9 )` 是线性相关的，因为 `2*x₁+3*x₂−2*x₃ = 0`。如果向量不是线性相关，则它们是**线性无关**的。例如，单位矩阵的列是线性无关的。  
