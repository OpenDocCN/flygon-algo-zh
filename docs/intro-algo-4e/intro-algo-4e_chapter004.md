...
如果我们限制常数`n`[0]和`c`足够大，使得对于`n` ≥ 2`n`[0]，`cn`支配了被 `Θ(n)`项隐藏的匿名函数，那么最后一步成立。
...
因为 `a`, `b`, `n`[0] 和 `ϵ` 都是常数。函数 `f`^′(`n`) 满足引理 4.3 的情况 1 的条件，并且引理 4.2 中方程 (4.18) 的求和结果为 ![art](img/Art_P208.jpg)。由于 `a`, `b` 和 `n`[0] 都是常数，我们有
