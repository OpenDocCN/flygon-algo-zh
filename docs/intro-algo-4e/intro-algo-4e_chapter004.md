...
满足多项式增长条件的函数示例包括任何形式为`f(n) = Θ(n^α lg^β n lg lg^γn)`的函数，其中α、β和γ是常数。本书中使用的大多数多项式有界函数满足这个条件。指数和超指数不满足（例如，参见练习 4.7-2），也存在一些不满足多项式有界条件的函数。  
